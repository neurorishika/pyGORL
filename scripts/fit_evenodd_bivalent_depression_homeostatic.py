...
def loglik(params, choices, rewards):
    ...
    hr, lr, up_dr, fb_trans, fb_up, mu_inh, fb_syn = params
    # initialize the mushroom body
    sum_log_lik = 0

    def sub(i):
        MB = MushroomBody(
            hr=hr, lr=lr, up_dr=up_dr, fb_trans=fb_trans, fb_up=fb_up, mu_inh=mu_inh, fb_syn=fb_syn, **ignored_params
        )
        upwind_drives = []
...
def fit_MB(choices, rewards):
    ...
    eps = 1e-3
    params_init = np.array([5.0, 0.5, 5.0, 0.5, 0.5, 0.5, 0.5])
    params_bounds = [
        (eps, 100),
        (eps, 1 - eps),
        (eps, 100),
...
# Dump to disk
with gzip.open(args.output + "fit_results/bivalent_depression_homeostatic_fit_results.pkl.gz", "wb") as f:
